...
<div class="wrapper col5">
  <div id="container">
    ...
    ResultSet userdetails=OutletAdminDAO.getUSerDeatilsByUSerID(userID);
    ...
    %>
      ...
      <form action="confirmpurchase.jsp">
      <input type="hidden" name="user" value="<%=userID%>">
						<table>
						<%if(userdetails.next()){ %>
							<tr>
								<td>Firstname:</td>
								<td><%=userdetails.getString("firstname") %></td>
							</tr>
							<tr>
								<td>Lastname:</td>
								<td><%=userdetails.getString("lastname") %></td>
							</tr>
							<tr>
								<td>age:</td>
								<td><%=userdetails.getString("age") %></td>
							</tr>
							<tr>
								<td>Gender:</td>
								<td><%=userdetails.getString("gender") %></td>
							</tr>
							<tr>
								<td>Address:</td>
								<td><%=userdetails.getString("address") %></td>
							</tr>
							<tr>
								<td>Phone:</td>
								<td><%=userdetails.getString("phone") %></td>
							</tr>
							
							<%for(int i=0;i<VerifyProduct.prod.length;i++){ %>
								<tr>
									<td>product.<%=i %></td>
									<td><%=VerifyProduct.prod[i] %></td>
								</tr>
							<%} %>
						<%} %>
						<tr>
